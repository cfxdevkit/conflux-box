...
const GENERATED = path.resolve(process.cwd(), 'src/data/generatedContractTemplates.ts');
...
function loadGeneratedSync() {
  ...
  const body = content.replace(/export const generatedContractTemplates = /, '').replace(/as const;\s*$/, '');
  ...
  const script = new vm.Script(wrapped, { filename: 'generatedContractTemplates.vm.js' });
...
async function main() {
  ...
  if (!contractName) {
    console.error('Usage: SESSION=<sessionId> node scripts/deploy-from-generated.js <ContractName> <chain> <args-json>');
    process.exit(1);
  }
  const session = process.env.SESSION;
  if (!session) {
    console.error('Set SESSION env var to the dev sessionId (Authorization: Bearer <session>)');
    process.exit(1);
  }
