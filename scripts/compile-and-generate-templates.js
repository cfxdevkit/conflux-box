...
const outFile = path.resolve(process.cwd(), 'src/data/generatedContractTemplates.ts');
...
function pickRuntimeBytecode(contractsOutput) {
  ...
  let runtimeBytecode = (contract.evm && contract.evm.deployedBytecode && contract.evm.deployedBytecode.object) || '';
  let fullBytecode = (contract.evm && contract.evm.bytecode && contract.evm.bytecode.object) || '';
  if (runtimeBytecode && !runtimeBytecode.startsWith('0x')) runtimeBytecode = '0x' + runtimeBytecode;
  if (fullBytecode && !fullBytecode.startsWith('0x')) fullBytecode = '0x' + fullBytecode;
...
function generateTsModule(results) {
  const lines = [];
  lines.push('// This file is generated by scripts/compile-and-generate-templates.js');
