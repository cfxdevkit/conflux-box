...
export default function Protocols() {
  ...
  return (
    <Stack gap="xl">
      <div>
        <Title order={2} mb="xs">DeFi Protocols</Title>
        <Text c="dimmed">
      ...
      <Group>
        ...
        <Badge variant="outline" size="lg">
          {availableIntegrations.length} Available Integration{availableIntegrations.length !== 1 ? 's' : ''}
        </Badge>
      </Group>
          ...
          <Card key={integration.metadata.id} withBorder radius="md" p="md">
            <Stack gap="sm">
              <Group justify="space-between">
                <Group gap="sm">
                  <ThemeIcon size="md" radius="md" variant="light" color={getCategoryColor(integration.metadata.category)}>
                    {integration.metadata.icon}
                  </ThemeIcon>
                  <div>
                    <Text fw={600} size="sm">{integration.metadata.name}</Text>
                    <Text size="xs" c="dimmed">{integration.metadata.description}</Text>
                  </div>
                </Group>
              </Group>
      ...
      {availableIntegrations.length === 0 && (
        <Alert icon={<IconInfoCircle size={16} />} color="blue">
          No protocols available for the current network. Switch to testnet or mainnet to access DeFi protocols.
        </Alert>
      )}
      ...
      {availableIntegrations.length > 0 && (
        <>
          <Title order={3} mt="xl">Interactive Protocols</Title>
      ...
      {Object.keys(integrationsByCategory).length > 0 && (
        <>
          <Title order={3} mt="xl">All Protocol Categories</Title>
          
          <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md">
            {(Object.entries(integrationsByCategory) as [IntegrationType, Integration[]][]).map(([category, integrations]) => (
                ...
                <Stack gap="sm">
                  <Group>
                    ...
                    <Text size="sm" c="dimmed">
                      {integrations.length} protocol{integrations.length !== 1 ? 's' : ''}
                    </Text>
                  </Group>
                  ...
                  <Stack gap="xs">
                    {integrations.map((integration) => (
                      <Group key={integration.metadata.id} justify="space-between">
                        ...
                        {integration.metadata.tvl && (
                          <Text size="xs" c="dimmed">{integration.metadata.tvl}</Text>
                        )}
                      </Group>
                    ))}
                  </Stack>
      ...
      <Text size="xs" c="dimmed" ta="center" mt="xl">
        Integrations are modular and can be easily added or removed. 
        Each integration is network-aware and will only show when compatible.
      </Text>
    </Stack>
  );
}