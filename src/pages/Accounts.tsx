...
export default function Accounts() {
  ...
  const selectedAccountData = accounts?.[parseInt(selectedAccount)] || accounts?.[0];
      ...
      {(!accounts || accounts.length === 0) && !isLoading ? (
        <Card withBorder padding="xl" radius="md">
          <Stack align="center" gap="sm">
            ...
            <Text size="sm" c="dimmed">Start the DevKit node to initialize accounts</Text>
          </Stack>
        </Card>
      ) : (
        <Card withBorder padding="lg" radius="md">
              ...
              <Stack gap="md">
                  ...
                  <Select
                    ...
                    data={
                      accounts?.map((account: any, index: number) => ({
                        ...
                        label: `Account ${index} (${(account.addresses?.core || account.address)?.slice(0, 10)}...)`,
                      })) || []
                    }
                  />
                </Group>
                ...
                {selectedAccountData && (
                  <Card withBorder padding="md" bg="gray.0">
                    <Group justify="space-between" mb="sm">
                      ...
                      <Group gap="xs">
                        {parseInt(selectedAccount) === 0 && (
                          <Badge size="sm" color="red">Admin</Badge>
                        )}
                    ...
                    <Stack gap="sm">
                      <Group justify="space-between">
                        <Text size="sm" c="dimmed">Core Address:</Text>
                        <Group gap="xs">
                          <Code style={{ fontSize: '11px' }}>
                            {selectedAccountData.addresses?.core || selectedAccountData.address}
                          </Code>
                          <CopyButton value={selectedAccountData.addresses?.core || selectedAccountData.address}>
                            {({ copied, copy }) => (
                      ...
                      {selectedAccountData.addresses?.evm && (
                        <Group justify="space-between">
                          <Text size="sm" c="dimmed">eSpace Address:</Text>
                          <Group gap="xs">
                            <Code style={{ fontSize: '11px' }}>
                              {selectedAccountData.addresses.evm}
                            </Code>
            ...
            <Tabs.Panel value="transfer" pt="md">
              <Stack gap="md">
                ...
                <NumberInput
                  ...
                  onChange={(value) => setTransferAmount(typeof value === 'string' ? parseFloat(value) || '' : value)}
                  disabled={sendMutation.isPending}
                  min={0}
                  step={0.1}
                  decimalScale={6}
                ...
                {signature && (
                  <Card withBorder padding="md" bg="gray.0">
                    <Group justify="space-between" mb="xs">
                      <Text size="sm" fw={500}>Signature Result</Text>
                    ...
                    <Group gap="xs" mt="xs">
                      <Badge size="xs" color="blue">Chain: {signChain}</Badge>
                      <Badge size="xs" color="green">Account: {selectedAccount}</Badge>
                      <Badge size="xs" color="purple">Length: {signature.length}</Badge>
                    </Group>
                  </Card>
                )}
              </Stack>
